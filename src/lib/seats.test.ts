...
// 模拟线下场景：4 号后插入旅行者 15 号（1 2 3 4 15 5 6 … 14）
const game15 = (): Seat[] => [
...
describe('nextSeatNumber（锚号只增不重用）', () => {
  it('含高位旅行者号时新号 = max+1', () => {
    expect(nextSeatNumber(game15())).toBe(16);
  });
  it('空桌 = 1', () => {
...
describe('addSeat / removeSeat（旅行者进出）', () => {
  it('新座位编号 = max+1，displayOrder 可插中间', () => {
    const after = addSeat(game15(), { playerName: '新旅', roleId: 'bureaucrat', isTraveler: true }, 5.5);
    const added = after.find((s) => s.playerName === '新旅')!;
    expect(added.seatNumber).toBe(16); // 不复用、不顺序填补
    expect(added.isTraveler).toBe(true);
  });

  it('移除后编号退役：后续新增不复用旧号', () => {
    const removed = removeSeat(game15(), 15)!;
    expect(removed).toHaveLength(5);
    expect(removed.some((s) => s.seatNumber === 15)).toBe(false);
    const after = addSeat(removed, { playerName: '又一位', roleId: 'scapegoat', isTraveler: true });
