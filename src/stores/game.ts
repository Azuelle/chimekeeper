...
import { deleteGame, loadCurrentGame, loadGame as loadGameById, saveGame } from '../persistence/repo';
...
interface GameState {
  ...
  /** 登记死亡（任意原因），默认保留一票投票权 */
  registerDeath(seatNumber: number, cause?: string): void;
  /** 登记复活 */
  ...
  return {
    ...
    registerNomination(nominatorSeat, nominatedSeat) {
      ...
      useEventStore
        .getState()
        .append(
          createEvent(game, 'nomination', { seatNumbers: [nominatorSeat, nominatedSeat], payload: { nominatorSeat, nominatedSeat } }),
        );
    },
